...
impl WorkspaceRegistry {
    ...
    pub fn save_code_metrics(
        ...
        conn.execute(
            ...
            rusqlite::params![repo_id, total_lines as i64, source_lines as i64, test_lines as i64, 
                              comment_lines as i64, file_count as i64, language_breakdown, 
                              chrono::Utc::now().to_rfc3339()],
        )?;
        Ok(())
    }

    pub fn get_code_metrics(conn: &rusqlite::Connection, repo_id: &str) -> anyhow::Result<Option<CodeMetrics>> {
        let mut stmt =
        ...
        if let Some(row) = rows.next()? {
            ...
            let language_breakdown = serde_json::from_str(&language_breakdown).unwrap_or(serde_json::Value::Null);
    ...
    pub fn list_code_metrics(conn: &rusqlite::Connection) -> anyhow::Result<Vec<(String, CodeMetrics)>> {
        let mut stmt = conn.prepare(
