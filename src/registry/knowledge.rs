...
mod tests {
    ...
    fn test_module_crud() {
        ...
        WorkspaceRegistry::save_module(&conn, "repo-a", "main", "binary", "src/main.rs").unwrap();
        WorkspaceRegistry::save_module(&conn, "repo-a", "lib", "library", "src/lib.rs").unwrap();

        let modules = WorkspaceRegistry::list_modules(&conn, "repo-a").unwrap();
        assert_eq!(modules.len(), 2);

        WorkspaceRegistry::clear_modules(&conn, "repo-a").unwrap();
        let empty = WorkspaceRegistry::list_modules(&conn, "repo-a").unwrap();
    ...
    fn test_paper_roundtrip() {
        ...
        WorkspaceRegistry::save_paper(&conn, &paper).unwrap();
        let papers = WorkspaceRegistry::list_papers(&conn).unwrap();
