...
impl super::WorkspaceRegistry {
    ...
    pub fn delete_known_limit(conn: &rusqlite::Connection, id: &str) -> anyhow::Result<bool> {
        delete_known_limit(conn, id)
    }
    pub fn resolve_known_limit(conn: &rusqlite::Connection, id: &str) -> anyhow::Result<bool> {
        resolve_known_limit(conn, id)
    }
    pub fn seed_hard_vetoes(conn: &rusqlite::Connection) -> anyhow::Result<usize> {
        seed_hard_vetoes(conn)
    }
}
...
mod tests {
    ...
    fn test_known_limit_crud() {
        ...
        save_known_limit(&conn, &limit).unwrap();
        let fetched = get_known_limit(&conn, "test-limit-1").unwrap().unwrap();
        ...
        assert!(deleted);
        assert!(get_known_limit(&conn, "test-limit-1").unwrap().is_none());

        // Verify oplog entries
        let oplog = crate::registry::workspace::list_oplog(&conn, 10).unwrap();
