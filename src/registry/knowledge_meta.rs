...
mod tests {
    ...
    fn test_knowledge_meta_crud() {
        ...
        WorkspaceRegistry::save_knowledge_meta(&conn, &meta).unwrap();
        let fetched = WorkspaceRegistry::get_knowledge_meta(&conn, "meta-1").unwrap().unwrap();
        ...
        assert!(deleted);
        assert!(WorkspaceRegistry::get_knowledge_meta(&conn, "meta-1").unwrap().is_none());
    }
}
