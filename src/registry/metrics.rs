...
mod tests {
    ...
    fn test_save_and_get_code_metrics() {
        ...
        save_code_metrics(&conn, "repo-a", &metrics).unwrap();

        let fetched = get_code_metrics(&conn, "repo-a").unwrap().unwrap();
    ...
    fn test_get_code_metrics_missing() {
        ...
        let result = get_code_metrics(&conn, "missing").unwrap();
    ...
    fn test_list_code_metrics() {
        ...
        m2.total_lines = 500;
        save_code_metrics(&conn, "repo-a", &m1).unwrap();
        save_code_metrics(&conn, "repo-b", &m2).unwrap();

        let all = list_code_metrics(&conn).unwrap();
        assert_eq!(all.len(), 2);
