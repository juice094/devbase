...
|:---|:---|
| `↑/↓` | 在仓库列表中导航 |
| `/` | **跨仓库代码搜索** — 在所有注册仓库中搜索文件内容（ripgrep 优先，自动回退到 walkdir） |
...
- **左侧 35%**：仓库列表，状态图标（⏳ 加载中 / 🔴 dirty / 🟡 behind / 🔵 ahead / 🟢 正常 / ⚪ 无远程）
- **右侧 65%**：四层详情面板
  1. **状态与元数据**：Git 状态、SyncPolicy、标签、语言、Stars
  2. **AI Insights**：智能洞察（diverged/dirty/stale/stars 增速/policy 提醒）
  3. **连接信息**：upstream URL、default branch、last sync
  4. **Stars Trend**：Sparkline 趋势图（最近 30 次 fetch 的历史）
