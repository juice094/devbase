...
| `devkit_skill_search` | **搜索 Skills** | "查找与代码审计相关的 skill" |
...
- **TUI 交互界面**：
  ...
  - 一键启动 gitui/lazygit
  - 智能同步建议
- **MCP Server**：34 个 tools（含 5 个 vault tools + 8 个代码分析工具 + 4 个 embedding/搜索工具 + 3 个 Skill Runtime tools + 1 个报告工具），stdio / SSE 双传输
...
- **Skill Runtime**：安装、发现、执行 AI Skills。内置 `embed-repo`、`search-workspace`、`knowledge-report`，支持从 Git URL 安装社区 Skill
