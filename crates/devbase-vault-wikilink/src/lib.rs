...
//! **职责**: 提取 Markdown 中的 `[[...]]` 风格 WikiLink，支持别名语法 `[[target|display]]`。
//! **边界**: 输入 `&str`（Markdown 内容），输出 `Vec<WikiLink>`。不触及文件系统或数据库。
//!
//! 与 devbase 的关系: 被 devbase `vault/indexer` 调用，建立笔记间链接图谱。
//!
//! Design decisions:
//! - 仅支持 `[[...]]` 语法: 兼容 Obsidian 标准，不支持 Markdown 原生链接。
//! - 别名分隔符为 `|`: 与 Obsidian 一致。
//! - 返回字符级位置 (start/end): 便于高亮和索引定位。
            ...
            while i < chars.len() && depth > 0 {
                if i + 1 < chars.len() && chars[i] == ']' && chars[i + 1] == ']' {
                    ...
                    if depth == 0 {
                        ...
                        let link = parser::parse_link(&inner_str, start, i + 2);
                        links.push(link);
                        i += 2;
                        break;
                    }
