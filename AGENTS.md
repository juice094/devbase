...
- **当前阶段**：阶段三 — v0.11.2 已交付 / Phase 1 Stage 2 读路径迁移完成
- **当前版本**：v0.11.2（tagged）
...
| 主从表切换 | 🟡 | Stage 2 读路径迁移完成，全部 SELECT 已切到 `entities` | `entities` 为第一公民 | Stage 3 删除 `repos` 表（需先移除 13 个子表 FK + 验证零引用） | v0.12.0 |
